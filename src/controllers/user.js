...
export class UsersController {
  static async register (req, res) {
    ...
    const result = await UsersModel.register({ input: userInfo })
  ...
  static async deleteUser (req, res) {
    ...
    const result = await UsersModel.deletUser({ input: userInfo })
    if (!result.success) return res.status(result.statusCode).json({ message: result.message })
    res.json(result.message)
  }
  ...
  static async verifyUser (req, res) {
    ...
    const result = await UsersModel.verifyUser(token)
